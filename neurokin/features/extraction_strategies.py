...
class SampleFeatureExtractionStrategy(FeatureExtraction):
    ...
    def _run_feature_extraction(self, source_marker_ids: List[str]) -> pd.DataFrame:
        ...
        if self.params['aggregation_method'] == 'mean':
            filtered_df = filtered_df.rolling(self.params['window_size']).mean()
        elif self.params['aggregation_method'] == 'sum':
            filtered_df = filtered_df.rolling(self.params['window_size']).sum()
        else:
            raise NotImplementedError('The SampleFeatureExtractionStrategy does not yet include an implementation '
                                      f'to aggregate the data using the requested "{self.params["aggregation_method"]}" '
        ...
        filtered_df = self._rename_columns_on_selected_idx_level(df = filtered_df,
                                                                 suffix = f'_sliding_{self.params["aggregation_method"]}')
        return filtered_df