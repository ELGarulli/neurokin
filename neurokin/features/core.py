...
class DefaultParams:
    ...
    def assert_input_params_and_fill_with_defaults(self, input_params: Optional[Dict[str, Any]]=None) -> Dict[str, Any]:
        if input_params == None:
            ...
            assert type(input_params) == dict, f'input_params has to be a dictionary, not: {input_params}.'
            for input_key, input_value in input_params.items():
                ...
                invalid_value_type_message = (f'input_params[{input_key}] has to be one of the following types: {self.types[input_key]}.'
                                              f'However, you passed: {input_value}, which is of type: {type(input_value)}')
                assert type(input_value) in self.types[input_key], invalid_value_type_message
    ...
    def _assert_identical_keys(self, value_keys: List[str], type_keys: List[str]) -> None:
        keys_dont_match_message = ('The keys of values & types have to be identical! However, "{}" '
                                   'of {} does not match with any key in {}: {}.')
        for key in value_keys:
            assert key in type_keys, keys_dont_match_message.format(key, 'values', 'types', type_keys)
        for key in type_keys:
            assert key in value_keys, keys_dont_match_message.format(key, 'types', 'values', value_keys)
    ...
    def _assert_values_in_types_are_lists_of_types(self, types: Dict[str, List[type]]) -> None:
        for type_key, list_of_types in types.items():
            ...
            for elem in list_of_types:
                assert type(elem) == type, f'The element "{elem}" in types[{key}] is not a type!'
                
                
    def _assert_default_values_match_valid_types(self, values: Dict[str, Any], types: Dict[str, List[type]]) -> None:

        for key, default_value in values.items():
            invalid_default_value_type_message = (f'The default value for "{key}": {default_value}, is of '
...
class FeatureExtraction(ABC):
    ...
    def __init__(self, sliced_marker_df: pd.DataFrame, params: Optional[Dict[str, Any]]=None) -> None:
        self.default_params = self._initialize_default_params()
        self.marker_df = sliced_marker_df
        self.params = self.default_params.assert_input_params_and_fill_with_defaults(input_params = params)
    ...
    def extract_features(self, source_marker_ids: List[str]) -> pd.DataFrame:
    ...
    def _copy_filtered_columns_of_df(self,
        ...
        idx = pd.IndexSlice
        filtered_df = df_to_filter.loc[:,idx[scorer_filter, marker_id_filter, coords_filter]].copy()
        return filtered_df
    
    
    def _rename_columns_on_selected_idx_level(self,
                                              df: pd.DataFrame,
                                              column_idx_level: Union[str, int]=2, # refers to coords level; 'coords' would work the same
                                              prefix: str = '',
                                              suffix: str = ''
                                             ) -> pd.DataFrame:
        """
        ...
        """
        current_column_names = list(df.columns.get_level_values(column_idx_level).unique())
        new_column_names = [f'{prefix}{column_name}{suffix}' for column_name in current_column_names]
