...
def plot_spectrogram(ax: ax, fs: float, raw: ArrayLike, title: str, ylim: List[int], tick_spacing: int = 10,
                     nfft: int = None, noverlap: int = None, **kwargs) -> ax:
    """
    Wrapper for matplotlib spectrogram to add some style elements.
    :param ax: axes where to plot
    :param fs: sampling frequency
    ...
    """
    ax.set_title(title)
    pxx, freq, t, im = ax.specgram(raw, NFFT=nfft, Fs=fs, noverlap=noverlap, **kwargs)
    ax.set_ylim(ylim)
    ax.set_yticks(np.arange(ylim[0], ylim[1], tick_spacing))
    return ax, im
...
def plot_welch(ax: ax, freqs: ArrayLike, pxx_den: ArrayLike, title: str, xlim: List[int], ylim: List[int],
               tick_spacing: int = 10, label: str = None) -> ax:
    """
    Returns a plot of psd with some style elements
    :param ax: axes where to plot
    :param freqs: array like of frequencies corresponding to the x axis
    ...
    :param ylim: limit of power for plotting
    :param tick_spacing:  spacing of the ticks on the axis
    :param label: label of plotted data
    :return:
    """
    ax.set_title(title)
    ax.set_xlim(xlim)
    ...
    ax.semilogy(freqs, pxx_den, label=label)
    return ax
